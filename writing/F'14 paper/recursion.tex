...
\begin{figure*}[t]
  \centering
  \includegraphics[width=5in]{decayfig.eps}
%  \includegraphics[width=5.3in]{recursion\string_pairwise.eps}
  \caption{\mynote{Model performance on propositional logic, by expression size. }
...
%    failing to capture the meaning contrasts.
  ...
  \label{prop-results}
  % TODO: Regenerate figure from final model.
\end{figure*}
...
between test accuracy and statement length. \mynote{We found that both 
models were able to perform well on unseen small test examples, 
with RNN accuracy above
97\% and RNTN accuracy above 99\% on formulae below length five.
Starting at size four, performance gradually falls with increasing
size. Training accuracy was 99.4\% for the RNN and 99.9\% for the RNTN.}
...
logic. It appears that these approximations are accurate enough to yield
...
RNN, and this much more capacity to memorize a large number of narrow 
generalizations that work on the test data but don't generalize as well. In addition,
the RNTN used here used slightly less L2 regularization than the 
RNN ($\lambda_{RNN} = 0.001$, $\lambda_{RNTN} = 0.0003$), as had
yielded better performance earlier in tuning.
