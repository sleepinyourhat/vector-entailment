...
representations, \mynote{and the RNTN model was approximately optimal with 25
dimensions. We found that the both models model were able to perform
well on unseen small test examples, with RNN accuracy above
97\% and RNTN accuracy above 98\% on formulae below length five.
Starting at size four, performance gradually falls with increasing
size. Training accuracy was 99\% for the RNN and 99.5\% for the RNTN.}

The performance of these model on small unseen test examples
...
\mynote{In an effort to better understand how the models are decaying, 
