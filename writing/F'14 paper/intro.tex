...
structures. Other recent work
\cite{grefenstette2013towards,rocktaschellow} has developed rich
frameworks for computing the meanings of complex fragments of natural
language compositionally from vector or tensor representations, but
these approaches do not confront the question of how to learn their
highly structured representations from data using existing applied
machine learning models.

% TODO: Any way to avoid nested parens? Yes, use \citealt ---CP

Our research uses the task of \ii{natural language
...
% TODO: Do we need both of these Icard/Moss citations? -- I'd prefer
% to keep them ---CP.
...
of two classes of neural model---plain RNNs and recursive neural
tensor networks (RNTNs; \cite{socher2013acl1})---can learn those
...
quantified statements like \ii{every reptile walks} and \ii{no turtle
  moves}. \tbchecked{We find that the plain recursive NN achieves only
  mixed results in all three experiments, whereas the stronger RNTN
...
\mynote{CONSIDER ADDING A CONCRETE DETAIL --- E.G., ACCURACY
  AND ITS RELATION TO THE COMPETITION MEDIAN}, suggesting that the
representational abilities that we observe in the previous sections
are not limited to carefully circumscribed tasks.
