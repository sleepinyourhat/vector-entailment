...
% TODO: TreeRNN.
% TODO: More language about the representations being the object of study.
% TODO: Is there any way to squeeze a useful example into the first page?
% TODO: Clarify early that we study vector pair classification. Maybe move up fig. 1?
% TODO: Clarify that we aren't implementing NatLog rules.
% TODO: ``Something that does that'' epigram in intro.
% TODO: Mention tree pair archichture.

Supervised recursive neural network models (RNNs; \citealt{goller1996learning}) for sentence meaning
...
the ability of these models to learn compositional semantic grammars, though many remain skeptical of the extent to which vector space representations can model complex compositional semantics, and it remains an open question whether learned can achieve the
high-fidelity distributed representations of recent algebraic
approaches to vector space modeling \cite{ClarkCoeckeSadrzadeh2011,grefenstette2013towards,Hermann-etal:2013,rocktaschellow},
...
% TODO: Condense `skeptical' sentence
...
entirely from data in a typical machine learning settings. 
...
structures. 
% TODO: Note about how their reps are learned instead.
...
language. We use the powerful
 natural logic developed by
...
from the lexicon up to complex phrases. The formal properties of this system are now well-understood
\cite{Icard:Moss:2013,Icard:Moss:2013:LILT}.

% TODO: Cut the below para. Replace with a more high-level summary of how we are paralleling NatLog, and how compositionality fits with that (Q: Does it?).
...
(\S\ref{sec:join}), we evaluate the ability of these models to
learn the core relational algebra of natural logic from data. Our
second experiment (\S\ref{sec:recursion}) extends this
evaluation to relations between complex recursive structures like
$(a \plor b)$ and $\plneg (\plneg a \pland \plneg b)$, and our third
experiment (\S\ref{sec:quantifiers}) covers relations between
statements with quantification and negation like \ii{every reptile walks} and \ii{no turtle
 moves}. Though the performance of the plain RNN model 
 is somewhat poor in our first experiment, we find that the stronger RNTN model
  generalizes well in every case, suggesting that it has learned to simulate our target logical concepts. % TODO: Split into two sentences
...
surpassing several submitted models with significant hand-engineered task-specific features.
...
\begin{table*}[tp]
  ...
  \begin{tabular}{l c l l} 
    ...
    \bottomrule
  \end{tabular}
  \caption{\label{b-table}The seven natural logic relations of MacCartney and Manning \protect\shortcite{maccartney2009extended} are defined abstractly on pairs of sets drawing from the universe  $\mathcal{D}$, and can be applied to any pair of words, phrases, or sentences according to standard approaches to semantic interpretation. % TODO: Condense, leave in reference to denotation.
