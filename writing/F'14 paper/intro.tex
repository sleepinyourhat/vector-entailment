...
% TODO: Can we fit a version of the ``find something that does that'' quote in intro somewhere?

% TODO: This para might be a bit repetitive.
...
though many remain skeptical of the extent to which they can learn to represent
the necessary complex compositional structures using fixed-size vectors. 
In particular, it remains an open question whether learned models can achieve the
...
entirely from data in typical machine learning settings. 
...
While one of our experiments uses natural logic only as an approximate
description of the desired model behavior, three experiments draw on this
 tradition much more directly, and
test models' abilities to learn to reproduce the precise behavior of the natural logic developed by
\newcite{maccartney2009extended}: one
covers learning to reason with the bare logical relations (\S\ref{sec:join}), a second covers reasoning with statements constructed compositionally from recursive functions (\S\ref{sec:recursion}),
and a third covers the additional complexity that results from quantification (\S\ref{sec:quantifiers}).
