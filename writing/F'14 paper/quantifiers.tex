...
In each run, we partition the full set of possible sentences under the grammar into training and test sentences, and label all of the pairs from within each set to generate a training set of 27k pairs and a test set of 7k pairs. Because the model doesn't see the test sentences at training time, it cannot directly use the kind of reasoning described in \S\ref{sec:join}, each model must learn a
