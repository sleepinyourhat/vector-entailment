...
\begin{table}[htp]
  ...
  \begin{tabular}{ l rrr }
    ...
    \midrule
    \textsc{all split}	& 35.4 &	65.3&	\textbf{99.0}
    \\[1ex]    
    \textsc{pair two/less-than-two}	& 59.8 &	77.6 &	\textbf{100} \\
    \textsc{pair not-all/not-most}	&0 & 60.8 &	\textbf{82.0} \\
    \textsc{pair all/some}	& 0& 52.5  &	\textbf{85.1} \\
    \textsc{pair no/no}	& 30.8 &	68.9 &	\textbf{99.9} \\
    \bottomrule
  \end{tabular}
...
After some cross-validation, we chose 20-dimensional word and phrase representations, and 75 dimensional feature vectors for the classifier.
% TODO: Update dimensionality

The RNN performed poorly at this task,
even though the sentences used in these examples are short enough
to avoid the pathology shown in Figure~\ref{prop-falloff}.
However, the perfect performance by the RNTN on the
\textsc{pair no/no} experiment and its strong performance on
\textsc{all split} and \textsc{pair two/less-than-two} 
suggests that it is at least plausible
that this stronger model is able to handle quantifiers correctly given
