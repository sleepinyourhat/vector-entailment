...
\scalebox{0.85}{
 \begin{tikzpicture}
  ...
  \end{tikzpicture}}
        ...
        \caption{The classification architecture used in all NLI evaluations: for each model evaluated in Table~\ref{tab:nnresults}, two identical copies of the model are run with the two sentences as input, and their outputs are used as the two 50d inputs shown here. In some transfer learning experiments (\S\ref{sec:transfer}), the premise embedding can be replaced with a zero vector for single-sentence tasks.}
