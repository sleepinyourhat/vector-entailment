...
For instance, the sentence pair \word{A boat sank in the Pacific Ocean} and \word{A boat sank in the Atlantic Ocean} could be labeled as a contradiction if the annotator assumes that the two events under discussion are coreferent, but could also be reasonably labeled as neutral if that assumption is not made. For a similar case with potentially different inutitions, \ii{Ruth Bader Ginsburg was appointed to the US Supreme Court} and \ii{I had a sandwich for lunch today} could be labeled as neutral if the annotator allows that they need not be refer to the same event, but is necessarily a contradiction if the annotator takes coreference to be required. Entity coreference presents a similar kind of uncertainty, as in the pair \word{A tourist visited New York} and \word{A tourist visited the city}. Assuming coreference between \ii{New York} and \ii{the city} justifies labeling the pair as an entailment, but without that assumption \ii{the city} could be taken to refer to a specific unknown city, leaving the pair neutral.
