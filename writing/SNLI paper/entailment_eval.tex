...
The results in \reftab{tab:transferresults} compare LSTM models trained each corpus separately with one trained on both using transfer. We tune the regularization parameter separately for each case. 

While training on either corpora alone yields poor performance, transfer yields the best performance yet reported on SICK for an unaugmented neural network model, surpasses the available EOP models, and approaches both the overall state of the art at 84.6\% \cite{lai2014illinois} and the 84\% level of interannotator agreement, which like represents an approximate performance ceiling. The randomly initialized model performs relatively poorly. This suggests that the introduction of a large high-quality corpus makes it newly possible to train representation-learning models for sentence meaning of a level of quality that is competitive with the best hand-engineered models on inference tasks.
