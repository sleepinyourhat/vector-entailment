...
Accuracies for two entailment algorithms included in the
  Excitement Open Platform.
Note that all accuracies are are on a 2-class
...
This yields a chance baseline of 66\% on our corpus, and 71\% on SICK.

Note that all results are reported without using any of the included lexical
  resources.
We do not attempt to necessarily compare against the state-of-the-art
  RTE systems as much as give a general sense for the relative
...
\midrule
\t{Lexicalized}            & \t{99.7}  & \b{78.2} \\ % & \t{78.7}
\t{Unigrams Only}          & \t{93.1} & \t{71.6} \\ % & \t{72.2}
\t{Unlexicalized}          & \t{49.4} & \t{50.4} \\ % & \t{50.1} % Last value was 50.39; rounded for consistency.
\bottomrule
\end{tabular}
\end{center}
% The caption
...
The latter of these was initially surprising, but could be attributed to
  the classifier automatically learning explicit negations or adjective
  modification.
A similar for bigram features in sentiment analysis was shown in
  \newcite{sidaw12simple}.
...
Although likely richer models would continue to improve performance,
  the results suggest that given enough data
  the more noisy part-of-speech overlap constraint can produce an effective
  model.
...
We test three sentence embedding models, each set to use 100d phrase and sentence embeddings. Our baseline sentence embedding simply sums the embeddings of the words in each sentence. In addition, we experiment with two simple sequence embedding models: a plain RNN and an LSTM RNN \cite{hochreiter1997long}. Both use the simple conventional layout with one layer per input token.
...
We compare the performance of an LSTM model with both the transfer initialization and a normal random initialization. We tune the regularization parameter separately for each case. \Reftab{tab:transferresults} shows these results. 
