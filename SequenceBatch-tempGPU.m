        ...
        function [ endFeatures, connectionCosts, connectionAcc ] = runForward(sb, embeddingTransformMatrix, ~, compositionMatrix, hyperParams, trainingMode)
            ...
            for w = 1:sb.N
                ...
                elseif SUM
                    ...
                    if hyperParams.gpu
                        feat = ComputeRNNLayer(gpuArray(predActivations), gpuArray(sb.inputFeatures(:, :, w)), ...
                            gpuArray(compositionMatrix), @tanh);
                        sb.features(:, :, w) = feat;
                    else
                        feat = ComputeRNNLayer(predActivations, sb.inputFeatures(:, :, w), ...
            ...
            connectionCosts = 0;
            connectionAcc = -1;
            sb.features = gather(sb.features);
